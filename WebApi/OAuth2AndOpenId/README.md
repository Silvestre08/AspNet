...
Authorization code flow is vulnerable to code injection attacks. This means that an attacker got a hold of the authorization code of the victim and the code is a short term proof of identity that links the server session to the browser session.
The wat to mitigate this is to use the PKCE.
